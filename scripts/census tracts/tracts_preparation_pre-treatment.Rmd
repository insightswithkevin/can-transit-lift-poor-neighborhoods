---
title: "Pre-Treatment Data Wrangling - Census Tracts"
...
## Download of ACS detailed variables from 2013 for Los Angeles County, CA Census Tracts (Pre-Treatment)
...
la_tracts_2013 <- la_acs_orig %>%
  ...
  mutate(high_education = round(((B15003_022E + B15003_023E + B15003_024E + B15003_025E) / B15003_001E), digits = 2)) %>%
...
la_tracts_2013 <- la_tracts_2013 %>% 
  select(GEOID, NAME, geometry, medhinc_tract, medhinc_county, medhinc_ratio_tract, income_quintiles,
         high_inc_mobility, high_education, unemployment_rate, low_inc_hh, total_pop, median_age, median_rent) %>% 
  filter(complete.cases(medhinc_tract, medhinc_county, medhinc_ratio_tract, income_quintiles,
                        high_inc_mobility, high_education, unemployment_rate, low_inc_hh, total_pop, median_age, median_rent))
...
# Shapefile of LA census tracts

la_tracts_2013 <- st_as_sf(la_tracts_2013, coords = c("geometry"), crs = 26945)

la_tracts_2013 <-  st_transform(la_tracts_2013, crs = 26945)

st_write(la_tracts_2013, here("data/census tracts/outputs/la_tracts_2013.shp"), delete_layer=TRUE)


la_tracts_2013 <- st_read(here("data/census tracts/outputs/la_tracts_2013.shp"))

la_tracts_2013 <- la_tracts_2013 %>% 
  select(GEOID, NAME, geometry, medhinc_tract = mdhnc_t, medhinc_county = mdhnc_c,
         medhinc_ratio_tract = mdhnc__, income_quintiles = incm_qn, high_inc_mobility = hgh_nc_,
         high_education = hgh_dct, unemployment_rate = unmply_, low_inc_hh = lw_nc_h,
         total_pop = totl_pp, median_age = medin_g, median_rent = mdn_rnt)

ggplot() +
  geom_sf(data = la_tracts_2013)

st_crs(la_tracts_2013)

ggplot() +
  geom_sf(data = la_tracts_2013, fill = "khaki")


# Metro stations

file_paths <- c(
  here("data/census tracts/inputs/shapefiles/metro/230711_All_E-Line_Stations_Post_RC/230711_All_E-Line_Stations_Post_RC.shp"),
  here("data/census tracts/inputs/shapefiles/metro/230711_All_A-Line_Stations_Post_RC/230711_All_A-Line_Stations_Post_RC.shp"),
  here("data/census tracts/inputs/shapefiles/metro/802_805_RedPurple_Stations_0316/RedPurpleLine0316.shp"),
  here("data/census tracts/inputs/shapefiles/metro/803_Green_Stations_0316/GreenLine0316.shp"),
  here("data/census tracts/inputs/shapefiles/metro/230711_All_K-Line_Stations/230711_All_K-Line_Stations.shp")
)
...
ggplot() +
  geom_sf(data = la_tracts_2013, fill = "burlywood3") +
...
# Centroids of census tracts

la_tracts_centroids <- st_centroid(la_tracts_2013)

la_tracts_centroids <- st_transform(la_tracts_centroids, crs = 26945)

ggplot() + 
  geom_sf(data = la_tracts_centroids)


# Distance between centroids of census tracts and CBD (Central business district) of LA

la_cbd <-st_read(here("data/census tracts/inputs/shapefiles/la_cbd/Los_angeles_CBD.shp"))
...
distance_centroid_cbd <- st_distance(la_tracts_centroids, la_cbd)

distance_centroid_cbd <- as.data.frame(distance_centroid_cbd)


la_tracts_2013 <- cbind(la_tracts_2013, distance_centroid_cbd)

la_tracts_centroids <- cbind(la_tracts_centroids, distance_centroid_cbd)


# Centroids of census tracts within buffers
...
ggplot() +
  ...
  geom_sf(data = la_tracts_centroids, fill = NA, color = "black", alpha = 0.05)


buffer_centroids_intersect_new_e <- new_e_stations_buffer %>%
  st_join(., la_tracts_centroids, join = st_intersects) %>% 
...
# census tracts which are in the radius of new E-Line stations but not in buffer of the other metro lines
...
treated_la_tracts <- unique_observations_treated %>% 
  filter(metro_line == "e_line") %>% 
  filter(income_quintiles == 1 | income_quintiles == 2)



# census tracts which are not in the radius of any metro stations

dfs_control <-  list(la_tracts_2013, buffer_centroids_intersect_e, buffer_centroids_intersect_a, buffer_centroids_intersect_b_d, buffer_centroids_intersect_c)
...
control_la_tracts <- unique_observations_control %>%
  filter(income_quintiles == 1 | income_quintiles == 2)
...
treated_units <- la_tracts_2013 %>%
  filter(NAME %in% treated_la_tracts$NAME)

control_units <- la_tracts_2013 %>% 
  filter(NAME %in% control_la_tracts$NAME)
...
st_write(treated_units, here("data/census tracts/outputs/treated_units.shp"), delete_layer=TRUE)

st_write(control_units, here("data/census tracts/outputs/control_units.shp"), delete_layer=TRUE)
