---
title: "Post-Treatment Data Wrangling - Census Tracts"
...
## Download of ACS-5-Year-2017 detailed variables for Los Angeles County, CA Census Tracts (Post-Treatment)

```{r Variable download 1}

la_acs_orig_2017 <- get_acs(geography = "tract", # level of geography requested
...
la_tracts_2017 <- la_acs_orig_2017 %>% 
...
la_tracts_2017 <- st_transform(la_tracts_2017, crs = 26945)

```

## Extract from ACS-5-Year-2022 detailed variables for Los Angeles County, CA Census Tracts (Post-Treatment)

```{r Variable download 2}

la_acs_orig_2022 <- get_acs(geography = "tract", # level of geography requested
                            state ="006", # can use state FIPS codes as well e.g. 48
                            county = "037", # County FIPS codes are safer
                            year = 2022,
                            variables = c("B19013_001", # median household income
...
la_tracts_2022 <- la_acs_orig_2022 %>% 
  mutate(medhinc_tract = B19013_001E) %>% 
...
la_tracts_2022 <- st_transform(la_tracts_2022, crs = 26945)
...
matched_data_2013 <- matched_data_2013 %>% 
  select(GEOID, NAME, geometry, year, treatment, distance, weights, subclass,
         medhinc_tract = mdhnc_t, medhinc_county = mdhnc_c, medhinc_ratio_tract = mdhnc__,
         income_quintiles = incm_qn, high_inc_mobility = hgh_nc_, high_education = hgh_dct,
         unemployment_rate = unmply_, low_inc_hh = lw_nc_h, total_pop = totl_pp, median_age = medin_g,
         median_rent = mdn_rnt, distance_centroid_cbd = dstnc__)
...
matching_info_2017 <- la_tracts_2017 %>% 
  filter(GEOID %in% matched_data_2013$GEOID) %>%
  mutate(year = 2017) %>% 
  select(GEOID, NAME, geometry, medhinc_tract, medhinc_county, medhinc_ratio_tract, income_quintiles,
         high_inc_mobility, high_education, unemployment_rate, low_inc_hh, total_pop, median_age, median_rent, year)
  
matching_info_2017 <- left_join(matching_info_2017, matching_info_2013 %>% as.data.frame() %>% select(-geometry), by = "GEOID")


matching_info_2022 <- la_tracts_2022 %>% 
  filter(GEOID %in% matched_data_2013$GEOID) %>%
  mutate(year = 2022) %>% 
  select(GEOID, NAME, geometry, medhinc_tract, medhinc_county, medhinc_ratio_tract, income_quintiles,
         high_inc_mobility, high_education, unemployment_rate, low_inc_hh, total_pop, median_age, median_rent, year)
...
st_write(appended_df, here("data/census tracts/outputs/appended_df.shp"), delete_layer=TRUE)

st_write(appended_df_2013_2017, here("data/census tracts/outputs/appended_df_2013_2017.shp"), delete_layer=TRUE)

st_write(appended_df_2017_2022, here("data/census tracts/outputs/appended_df_2017_2022.shp"), delete_layer=TRUE)
